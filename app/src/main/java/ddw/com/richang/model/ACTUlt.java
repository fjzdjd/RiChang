...
public class ACTUlt{
    ...
    public String getPosterTop(){return this.posterTop;}
    public String getTime(){return this.time;}
    public String getTitle(){return this.title;}
    public String getPlace(){return this.place;}
    public long getCollectSum(){return this.collectSum;}
    public long getPraiseSum(){return this.praiseSum;}
    public long getReadSum(){return this.readSum;}
    public String getPay(){return this.pay;}
    public String getSize(){return this.size;}
    public String getSpeaker(){return this.speaker;}
    public String getSustain(){return this.sustain;}
    public IdNameMap getTheme(){return this.theme;}
    public ArrayList<IdNameMap> getTags(){return this.tags;}
    public String getTagsStr(){
        if(null==this.tags || 0==this.tags.size()) return "";
        String res="";
    ...
    public boolean isPlaned(){return this.planed;}
    public void changeCollected(final Activity activity, final Runnable succ,final Runnable error){
    ...
    public void addPlan(final PLAN plan,final Activity activity, final Runnable succ,final Runnable error){
        new Thread(new Runnable() {
            @Override
            public void run() {
                try{
                    ...
                    if(activity!=null){
                        if(200==obj.getInt("code")){
                            ...
                            if(0==PLAN.list.size()){
                                PLAN.list.add(plan);
                            }else if(plan.getDate4scroll().id>PLAN.list.get(PLAN.list.size()-1).getDate4scroll().id){
                                PLAN.list.add(plan);
                            }else if(plan.getDate4scroll().id<PLAN.list.get(0).getDate4scroll().id){
                                PLAN.list.add(0,plan);
                            }else if(1==PLAN.list.size()){
                                if(PLAN.list.get(0).getDate4scroll().id>plan.getDate4scroll().id){
                                    PLAN.list.add(0, plan);
                                }else{
                                    PLAN.list.add(plan);
                                }
    ...
    public ACTUlt(final long id){
        ...
        try{
            ...
            }else this.msg=obj.getString("msg");
        }catch(Exception e){
